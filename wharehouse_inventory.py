...
import sqlite3
#class for front end user (ul)
class Product:
    def __init__(self,root):
        
        #create object reference intence of Database class as p
        p = Database()
        p.conn()
        
        self.root = root
        self.root.title("WHAREHOUSE INVENTORY SALES PURCHASE MANAGEMENT SYSTEM")
        ...
        '''Lets call the database methods to perform database operation'''
        def close():
            print("Product : close method called")
            close = tkinter.messagebox.askyesno("WHAREHOUSE INVENTORY SALES PURCHASE MANAGEMENT SYSTEM","Really.....Do you want to close the system")
            if close > 0:
                root.destroy()
        ...
        def insert():
            ...
            if len(pId.get()) != 0:
                p.insert(pId.get(),pName.get(),pPrice.get(),pQty.get(),
                         pCompany.get(),pContact.get())
                productList.delete(0,END)
                productList.insert(END,pId.get(),pName.get(),pPrice.get(),pQty.get(),
                         pCompany.get(),pContact.get())
                
                showInProductList()#called showInProductList method
                #after inserting the data record to database
            
            else:
                tkinter.messagebox.askyesno("WAREHOUSE INVENTORY SALES\
                    PURCHASE MANAGEMENT SYSTEM","Really ...  Enter product id")
        ...
        #function resposible to show product table data to scroll productlist
        def showInProductList():
            print("product : showInProduct method called ")
            productList.delete(0,END)
            for row in p.show():
                productList.insert(END,row,str(""))
            print("Product : showInProductList method finished\n")
        # add to scroll bar
        def productRec(event):#function to be called from scroll from scrollbar product
        ...
        #function to delete the data record
        def delete():
        ...
        # search the record from data base table
        def  search():
        ...
        self.ITitle =Label(HeadFrame,font=('arial',50,'bold'),fg="red",text='Warehouse Inventory Sales Purchases ',bg='white')
        self.ITitle.grid()
        OperationFrame = Frame(MainFrame,bd=2 ,width=1300,height=60,padx=60,pady=20,bg='white',relief=RIDGE)
        ...
        RightBodyFrame = LabelFrame(BodyFrame,bd=2 ,width=400,height=400,padx=20,pady=10,bg='yellow',relief=RIDGE,font=('arial',15,'bold'),text='Product Item Information:')
